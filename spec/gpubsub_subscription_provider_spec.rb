...
describe Puppet::Type.type(:gpubsub_subscription).provider(:google) do
  before(:all) do
        ...
        context 'title == name' do
          ...
          context 'title == name (pass)' do
            before do
              ...
              expect_network_get_success_topic 1
              expect_network_get_success_topic 2
              expect_network_get_success_topic 3
            end

            let(:catalog) do
              apply_with_error_check(
                <<-MANIFEST
                gpubsub_topic { 'resource(topic,0)':
                  ensure     => present,
                ...
                gpubsub_topic { 'resource(topic,1)':
                  ensure     => present,
                ...
                gpubsub_topic { 'resource(topic,2)':
                  ensure     => present,
                ...
                gpubsub_subscription { 'title0':
                  ensure               => present,
                  ack_deadline_seconds => 1733817478,
                  push_config          => {
                    push_endpoint => 'test push_endpoint#0 data',
                  },
                  topic                => 'resource(topic,0)',
                  project              => 'test project#0 data',
                  credential           => 'cred0',
                }

                gpubsub_subscription { 'title1':
                  ensure               => present,
                  ack_deadline_seconds => 3467634957,
                  push_config          => {
                    push_endpoint => 'test push_endpoint#1 data',
                  },
                  topic                => 'resource(topic,1)',
                  project              => 'test project#1 data',
                  credential           => 'cred1',
                }

                gpubsub_subscription { 'title2':
                  ensure               => present,
                  ack_deadline_seconds => 5201452436,
                  push_config          => {
                    push_endpoint => 'test push_endpoint#2 data',
                  },
                  topic                => 'resource(topic,2)',
                  project              => 'test project#2 data',
                  credential           => 'cred2',
                }
                MANIFEST
            ...
            context 'Gpubsub_subscription[title0]' do
              subject do
                catalog.resource('Gpubsub_subscription[title0]').provider
              end
            ...
            context 'Gpubsub_subscription[title1]' do
              subject do
                catalog.resource('Gpubsub_subscription[title1]').provider
              end
            ...
            context 'Gpubsub_subscription[title2]' do
              subject do
                catalog.resource('Gpubsub_subscription[title2]').provider
              end
        ...
        context 'title == name (pass)' do
          ...
          subject do
            apply_with_error_check(
              ...
              MANIFEST
            ).catalog.resource('Gpubsub_subscription[title0]').provider.ensure
          end
        ...
        context 'title == name (pass)' do
          ...
          subject do
            apply_with_error_check(
              ...
              MANIFEST
            ).catalog.resource('Gpubsub_subscription[title0]')
              .provider.ensure
          end
  ...
  context '#flush' do
    subject do
      Puppet::Type.type(:gpubsub_subscription).new(
        ensure: :present,
        name: 'my-name'
      ).provider
    end
  ...
  def expect_network_get_success(id, data = {})
    ...
    expect(Google::Pubsub::Network::Get).to receive(:new)
  ...
  def expect_network_create(id, expected_body, data = {})
    ...
    expect(Google::Pubsub::Network::Put).to receive(:new)
  ...
  def expect_network_delete(id, name = nil, data = {})
    ...
    expect(Google::Pubsub::Network::Delete).to receive(:new)
  ...
  def expect_network_get_success_topic(id, data = {})
    id_data = data.fetch(:name, '').include?('title') ? 'title' : 'name'
    body = load_network_result_topic("success#{id}~" \
                                                           "#{id_data}.yaml")
           .to_json
    uri = uri_data_topic(id).merge(data)
  ...
  def load_network_result_topic(file)
    results = File.join(File.dirname(__FILE__), 'data', 'network',
                        'gpubsub_topic', file)
    raise "Network result data file #{results}" unless File.exist?(results)
    data = YAML.safe_load(File.read(results))
  ...
  # Only used for gpubsub_topic objects
  def uri_data_topic(id)
    {
  ...
  def self_link_topic(data)
    URI.join(
      'https://pubsub.googleapis.com/v1/',
      expand_variables_topic(
  ...
  def expand_variables_topic(template, data, ext_dat = {})
  ...
  def expand_variables(template, data, extra_data = {})
    Puppet::Type.type(:gpubsub_subscription).provider(:google)
