...
require 'google/pubsub/network/delete'
require 'google/pubsub/network/get'
require 'google/pubsub/network/post'
require 'google/pubsub/network/put'
...
Puppet::Type.type(:gpubsub_subscription).provide(:google) do
  mk_resource_methods
  ...
  def self.fetch_to_hash(fetch)
    {
      ...
      push_config: Google::Pubsub::Property::SubscriPushConfig.api_munge(
        fetch['pushConfig']
      ),
      ack_deadline_seconds: Google::Pubsub::Property::Integer.api_munge(
        fetch['ackDeadlineSeconds']
      )
    }.reject { |_, v| v.nil? }
  end
  ...
  def create
    debug('create')
    @created = true
    create_req = Google::Pubsub::Network::Put.new(self_link(@resource),
                                                  fetch_auth(@resource),
                                                  'application/json',
                                                  resource_to_request)
  ...
  def destroy
    debug('destroy')
    @deleted = true
    delete_req = Google::Pubsub::Network::Delete.new(self_link(@resource),
                                                     fetch_auth(@resource))
  ...
  def self.resource_to_hash(resource)
    {
      ...
      push_config: resource[:push_config],
      ack_deadline_seconds: resource[:ack_deadline_seconds]
    }.reject { |_, v| v.nil? }
  end
  ...
  def resource_to_request
    request = {
      name: @resource[:name],
      topic: @resource[:topic],
      pushConfig: @resource[:push_config],
      ackDeadlineSeconds: @resource[:ack_deadline_seconds]
    }.reject { |_, v| v.nil? }
  ...
  def self.return_if_object(response)
    ...
    result = decode_request(response)
  ...
  def self.fetch_resource(resource, self_link)
    get_request = ::Google::Pubsub::Network::Get.new(
